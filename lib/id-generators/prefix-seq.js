...
module.exports = function(option) {
  ...
  const sourceDir = option.context ? option.context.source_dir
    : join(__dirname, '../../test/source');
  ...
  return function() {
    ...
    if (fs.existsSync(postDir)) {
      files = files.concat(listDirSync(postDir));
    }
    if (fs.existsSync(draftDir)) {
      files = files.concat(listDirSync(draftDir));
    }
