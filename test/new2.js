...
const {init} = require('hexo-unique-post-path');

describe('new', () => {
  const hexo = new Hexo(pathFn.join(__dirname, 'new_test'), {silent: true});
  init(hexo);
  const n = hexo.extend.console.get('new2').bind(hexo);
  ...
  afterEach(() => fs.rmdir(hexo.source_dir));
  ...
  it('type - seq', () => {
    const date = moment(now);
    const path = pathFn.join(hexo.source_dir, '_posts', '1.md');
    const body = [
      'title: Hello World',
      'date: ' + date.format('YYYY-MM-DD HH:mm:ss'),
      'tags:',
  ...
  it('type - date-seq', () => {
    const date = moment(now);
    const path = pathFn.join(hexo.source_dir, '_posts', date.format('YYYYMMDD') + '01.md');
    const body = [
      'title: Hello World',
      'date: ' + date.format('YYYY-MM-DD HH:mm:ss'),
      'tags:',
